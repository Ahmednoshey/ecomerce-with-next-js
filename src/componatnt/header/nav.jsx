...
const Nav = ({ isRegister = false, isSignin = false, Admin = false }) => {
  ...
  if (status == "loading") {
    return <span className="loaderres">Loading</span>;
  }

  if (
    status == "authenticated" &&
    session.user.email == "ahmed2027@gmail.com"
  ) {
    return (
      <nav className="links flex">
        <Link
          style={{ marginLeft: "10px" }}
          className={`register ${Admin ? "border" : null}`}
          href="/add-product"
        >
  ...
  return (
    <nav className="links flex">
      {status == "unauthenticated" && (
        <>
          <Link
            className={`sign-in ${isSignin ? "border" : null}`}
            href="/signin"
          >
            <FontAwesomeIcon
              style={{ width: "0.7rem" }}
